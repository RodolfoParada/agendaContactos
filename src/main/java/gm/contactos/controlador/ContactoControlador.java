...
public class ContactoControlador {
    ...
    public String eliminar(@ModelAttribute("contacto") Contacto contacto){
        try {
            ...
            Contacto contactoExistente = contactoServicio.buscarContactoPorId(contacto.getIdContacto());
            if (contactoExistente != null) {
                contactoServicio.eliminarContacto(contacto);
                logger.info("Contacto eliminado exitosamente: " + contacto);
            } else {
                logger.warn("Intento de eliminar un contacto no existente: " + contacto);
            }
        } catch (Exception e) {
            logger.error("Error al intentar eliminar el contacto: " + contacto, e);
            // Puedes redirigir a una página de error o mostrar un mensaje de error
            return "error"; // Suponiendo que tienes una vista de error
        }
        return "redirect:/"; // Redirigir al controlador path "/"
    }
